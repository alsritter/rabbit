...
func init() {
	...
	confData := &conf.Data{
		Database: &conf.Data_Database{
			Dsn: "alsritter:xxxxxxxxxxxx@(dev.alsritter.icu:32131)/rabbit_dev?charset=utf8mb4&parseTime=True&loc=Local&timeout=60s",
		},
	}
	db := data.NewDB(confData, log.DefaultLogger)
	var err error
	_test_db, _, err = data.NewData(confData, log.DefaultLogger, db, nil)
	if err != nil {
